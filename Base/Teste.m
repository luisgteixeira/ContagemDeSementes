...
% original = imread('Base/feijao_original.png');
% binarizada = imread('Base/feijao.png');
...
%     vetor(4) = atributosB.Contrast;
% %     vetor(18) = atributosB.Homogeneity;
% %     vetor(19) = std(std(glcmB));
% %     vetor(20) = entropy(glcmB);     
%     vetor(5) = mean(mean(glcmB));
% 
% %     vetor(6) = length(find( binaria_cortada == 255 ));
%     
...
%     limiar = (20 * size(vetor_atributos,1)) / 100;
%     limiar = round(limiar);
%     
