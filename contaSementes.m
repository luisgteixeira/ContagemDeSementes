function [ imagem_resultante ] = contaSementes( original, binarizada )

    imagem_resultante = zeros(size(original));
    ...
    for i = 1 : n
        ...
        switch classe
            case 1
                img_aux = imagem_resultante(:,:,1);
                img_aux( find(img_label == i) ) = 255;
                imagem_resultante(:,:,1) = img_aux;

                amendoim = amendoim + 1;

            case 2
                img_aux = imagem_resultante(:,:,2);
                img_aux( find(img_label == i) ) = 255;
                imagem_resultante(:,:,2) = img_aux;

                azeitona = azeitona + 1;

            case 3
                img_aux = imagem_resultante(:,:,3);
                img_aux( find(img_label == i) ) = 255;
                imagem_resultante(:,:,3) = img_aux;
